...
LocalSMTP is a modern SMTP capture tool inspired by MailHog. It provides a Go 1.25 backend with a React + Vite UI, email-only login, and per-user inbox/sent views backed by SQLite.
...
4. Open `http://localhost:8025` and log in with any email.
5. Configure your app to use SMTP on `localhost:1025`.
...
| `DB_PATH` | `localsmtp.db` | SQLite database path |
| `AUTH_SECRET` | empty | Session signing secret (recommended in production) |

## MailHog reference

The original MailHog source is included under `MailHog/` as a reference.
